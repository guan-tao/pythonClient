...
def dataSend(countInt,bufList):
    ...
    print "point1:",len(bufList[0]),"point2:",len(bufList[1]),"point3:",len(bufList[2])
    if (len(bufList[0])>=43000) and (len(bufList[1])>=43000) and (len(bufList[2])>=43000):  #判断是缓存是否够200个包
...
#Server_Port4 = 10004
APServer_Port = 10004
Server_HOST1 = '127.0.0.1'  # 监测点1
Server_HOST2 = '127.0.0.1'  # 监测点2
Server_HOST3 = '127.0.0.1'  # 监测点3
APServe_HOST = '127.0.0.1'  # AP
BUFSIZ = 1024*1024*5
Server_ADDR1 = (Server_HOST1,Server_Port1)
Server_ADDR2 = (Server_HOST2,Server_Port2)
Server_ADDR3 = (Server_HOST3,Server_Port3)
APServe_ADDR = (APServe_HOST,APServer_Port)
...
#-----------------------------------接收buffer------------------------------------------
def readBuf(_socket,_buflist,index):
    while True:
        recvdata = _socket.recv(BUFSIZ)
        if recvdata:
            _buflist[index] += recvdata            
        elif  not recvdata:
            _socket.close()
            break
